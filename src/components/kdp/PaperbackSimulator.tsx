...
import { SlidersHorizontal, Euro, FileText, Target, MousePointer } from 'lucide-react';
...
export const PaperbackSimulator = ({ data, globalData, onChange }: PaperbackSimulatorProps) => {
  ...
  const precioSinIva = globalData.marketplace === 'ES' ? pvp / (1 + ivaPct / 100) : pvp;
  ...
  const getDiagnosticMessage = () => {
    ...
    if (clicsMaxPorVenta < 10) return { text: 'Margen muy ajustado para campañas de Ads.', color: 'text-warning' };
      ...
      <CardContent>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-muted/30 rounded-xl p-5 space-y-4">
            ...
            {/* Diagnostic Message */}
            <div className={`p-3 rounded-lg border ${diagnostic.color === 'text-success' ? 'bg-success/10 border-success/30' : diagnostic.color === 'text-warning' ? 'bg-warning/10 border-warning/30' : 'bg-destructive/10 border-destructive/30'}`}>
