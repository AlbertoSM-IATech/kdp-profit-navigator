...
import { FileText, TrendingDown, SlidersHorizontal, Check } from 'lucide-react';
...
export const PageSensitivityChart = forwardRef<HTMLDivElement, PageSensitivityChartProps>(
  ({ globalData, paperbackData, onPagesChange }, ref) => {
    ...
    const [simulatedPages, setSimulatedPages] = useState<number>(paperbackData.pages || 100);

    // Sync slider with actual pages when paperbackData changes
    useEffect(() => {
      if (paperbackData.pages && paperbackData.pages >= 24) {
        setSimulatedPages(paperbackData.pages);
      }
    }, [paperbackData.pages]);
    ...
    const sensitivityData = useMemo(() => {
      ...
      for (let pages = minPages; pages <= maxPages; pages += step) {
        ...
        data.push({
          ...
          isSimulatedPages: pages === simulatedPages || (pages >= simulatedPages - step/2 && pages <= simulatedPages + step/2),
        });
      }
      ...
      if (!data.some(d => d.isCurrentPages)) {
        ...
        if (printingResult.isValid) {
          ...
          data.push({
            ...
            margin: Math.round(margin * 10) / 10,
            isCurrentPages: true,
            isSimulatedPages: simulatedPages === currentPages,
          });
      ...
      return data;
    }, [canSimulate, paperbackData, globalData.marketplace, royaltyThreshold, simulatedPages]);
    ...
    const simulatedPoint = useMemo(() => {
      ...
      const { interior, size, pvp, ivaType, bookFormat } = paperbackData;
      const marketplace = globalData.marketplace;
      const printingResult = calculatePrintingCost(interior, size, simulatedPages, marketplace, bookFormat);
    ...
    }, [simulatedPages, canSimulate, paperbackData, globalData.marketplace, royaltyThreshold]);

    // Calculate max pages for slider
    const maxSliderPages = useMemo(() => {
      return Math.max((paperbackData.pages || 100) + 300, 600, 828);
    }, [paperbackData.pages]);

    const handleSliderChange = (value: number[]) => {
      setSimulatedPages(value[0]);
    };

    const handleApplySimulatedPages = () => {
      if (simulatedPages && onPagesChange) {
        onPagesChange(simulatedPages);
      }
    };
        ...
        <CardContent className="space-y-4">
          ...
          <div className="p-4 bg-muted/20 rounded-lg border border-border">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-medium">Simulador de páginas</span>
              {isSimulationDifferent && (
                <Badge variant="secondary" className="ml-auto">
                  Simulando: {simulatedPages} páginas
                </Badge>
              )}
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center text-xs text-muted-foreground">
                ...
                <span className="font-medium text-foreground">{simulatedPages} páginas</span>
                <span>{maxSliderPages} páginas</span>
              </div>
              
              {simulatedPoint && (
                <div className="grid grid-cols-3 gap-3 mt-3">
                  <div className="p-2 bg-background rounded text-center">
                    <span className="text-xs text-muted-foreground block">Coste impresión</span>
                    <span className="font-semibold">{simulatedPoint.printingCost.toFixed(2)}{currencySymbol}</span>
                  </div>
                  <div className="p-2 bg-background rounded text-center">
                    <span className="text-xs text-muted-foreground block">Regalía</span>
                    <span className={`font-semibold ${simulatedPoint.royalty >= 0 ? 'text-primary' : 'text-destructive'}`}>
                      {simulatedPoint.royalty.toFixed(2)}{currencySymbol}
                    </span>
                  </div>
                  <div className="p-2 bg-background rounded text-center">
                    <span className="text-xs text-muted-foreground block">Margen</span>
                    <span className={`font-semibold ${simulatedPoint.margin >= 30 ? 'text-primary' : simulatedPoint.margin >= 0 ? 'text-warning' : 'text-destructive'}`}>
                      {simulatedPoint.margin.toFixed(1)}%
                    </span>
                  </div>
                </div>
              )}
              
              {isSimulationDifferent && onPagesChange && (
                <Button 
                  variant="outline" 
                  size="sm" 
                  onClick={handleApplySimulatedPages}
                  className="w-full mt-2"
                >
                  <Check className="h-4 w-4 mr-2" />
                  Aplicar {simulatedPages} páginas al formulario
                </Button>
              )}
            </div>
          </div>
...
PageSensitivityChart.displayName = 'PageSensitivityChart';